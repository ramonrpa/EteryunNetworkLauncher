        ...
        val.onclick = (e) => {
            ...
            for (let i = 0; i < selectBtns.length; i++) {
                if (selectBtns[i].hasAttribute('selected')) {
                    ...
                    selectBtns[i].innerHTML = 'Selecionar conta'
                }
            }
            val.setAttribute('selected', '')
            val.innerHTML = 'Conta Selecionada &#10004;'
    ...
    Array.from(document.getElementsByClassName('settingsAuthAccountLogOut')).map((val) => {
        val.onclick = (e) => {
            ...
            if (Object.keys(ConfigManager.getAuthAccounts()).length === 1) {
                isLastAccount = true
                setOverlayContent(
                    'Aviso<br> Esta é a sua última conta',
                    'Para usar o Launcher, você deve estar logado em pelo menos uma conta. Será necessário fazer login novamente depois. <br><br>Tem certeza que deseja sair?',
                    'Tenho certeza',
                    'Cancelar'
                )
                setOverlayHandler(() => {
...
function refreshAuthAccountSelected(uuid) {
    Array.from(document.getElementsByClassName('settingsAuthAccount')).map((val) => {
        ...
        if (uuid === val.getAttribute('uuid')) {
            ...
            selBtn.innerHTML = 'Selecione a conta'
        }
    })
}
    ...
    Array.from(sEls).map((v, index, arr) => {
        v.onclick = () => {
            ...
            if (res) {
                ...
                setOverlayContent(
                    `Falha ao Excluir<br>Drop-in Mod ${fullName}`,
                    'Certifique-se de que o arquivo não esteja em uso e tente novamente.',
                    'OK'
                )
        ...
        if (dropinUI != null) {
            ...
            if (DropinModUtil.isDropinModEnabled(dropin.fullName) != dropinUIEnabled) {
                DropinModUtil.toggleDropinMod(CACHE_SETTINGS_MODS_DIR, dropin.fullName, dropinUIEnabled).catch(err => {
                    if (!isOverlayVisible()) {
                        setOverlayContent(
                            'Falha ao alternar<br>um ou mais mods drop-in',
                            err.message,
                            'OK'
                        )
                        setOverlayHandler(null)
...
function populateReleaseNotes() {
    $.ajax({
        url: 'https://github.com/ramonrpa/EteryunNetworkLauncher/releases.atom',
        success: (data) => {
...
function populateSettingsUpdateInformation(data) {
    if (data != null) {
        settingsUpdateTitle.innerHTML = `Nova ${isPrerelease(data.version) ? 'Pre-release' : 'Release'} disponivel`
        ...
        if (process.platform === 'darwin') {
            settingsUpdateButtonStatus('Baixar do GitHub<span style="font-size: 10px;color: gray;text-shadow: none !important;">Feche o inicializador e execute o dmg para atualizar.</span>', false, () => {
        ...
        settingsUpdateTitle.innerHTML = 'Você está executando a versão mais recente'
        settingsUpdateChangelogCont.style.display = 'none'
        populateVersionInformation(remote.app.getVersion(), settingsUpdateVersionValue, settingsUpdateVersionTitle, settingsUpdateVersionCheck)
        settingsUpdateButtonStatus('Verifique se há atualizações', false, () => {
            if (!isDev) {
                ipcRenderer.send('autoUpdateAction', 'checkForUpdate')
                settingsUpdateButtonStatus('Verificando atualizações..', true)
            }
        })
    }
}
