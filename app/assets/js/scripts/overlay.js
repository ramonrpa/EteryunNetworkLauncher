...
const skinUploadFileInput = document.getElementById('skinUploadFileInput')
// Bind skin select button
document.getElementById('skinUploadFileSelect').addEventListener('click', () => {
    skinUploadFileInput.click()
})

// Bind skin input change
skinUploadFileInput.addEventListener('change', async (e) => {
    if (e.target.files.length > 0) {
        let isValid = await validateImageSize(e.target.files[0])
        if (isValid) {
            document.getElementById('skinUploadFileValidate').style.display = 'none'
            let reader = new FileReader()
            reader.readAsDataURL(e.target.files[0])
            reader.onload = (e) => {
                document.getElementById('skinUploadImage').src = e.target.result
            }
            document.getElementById('skinUploadConfirm').disabled = false
        } else {
            document.getElementById('skinUploadFileValidate').style.display = 'block'
            skinUploadFileInput.value = ''
            document.getElementById('skinUploadConfirm').disabled = true
        }
    } else {
        document.getElementById('skinUploadConfirm').disabled = true
    }
...
document.getElementById('skinUploadConfirm').addEventListener('click', async (e) => {
    if (skinUploadFileInput.files.length > 0) {
        const accessToken = document.getElementById('skinUploadAccessToken').value
        const uuid = document.getElementById('skinUploadUUID').value
        const model = document.querySelector('input[name="model"]:checked').value
        Mojang.uploadSkin(skinUploadFileInput.files[0], model, uuid, accessToken).then(() =>
            setSkinUploadCallBackText('Sua skin foi alterada com sucesso.', '#0F0')
        ).catch(() =>
            setSkinUploadCallBackText('Ocorreu um erro ao alterar sua skin.', '#f00'))
    }
})

function setSkinUploadCallBackText(text, color, display = 'block') {
    document.getElementById('skinUploadCallBackText').innerHTML = text
    document.getElementById('skinUploadCallBackText').style.color = color
    document.getElementById('skinUploadCallBackText').style.display = display
}
...
document.getElementById('skinUploadCancel').addEventListener('click', () => {
    toggleOverlay(false)
    document.getElementById('skinUploadAccessToken').value = ''
    document.getElementById('skinUploadUUID').value = ''
    document.getElementById('skinUploadImage').src = './assets/images/skins/steve.png'
    document.getElementById('skinUploadConfirm').disabled = true
    skinUploadFileInput.value = ''
    setSkinUploadCallBackText('', '', 'none')
})
