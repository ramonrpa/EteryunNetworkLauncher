...
function updateSelectedServer(serv) {
    ...
    ConfigManager.save()
    server_selection_button.innerHTML = '\u2022 ' + (serv != null ? serv.getName() : 'Nenhum servidor selecionado')
    if (getCurrentView() === VIEWS.settings) {
...
const refreshServerStatus = async function (fade = false) {
    ...
    if (fade) {
        $('#server_status_wrapper').fadeOut(250, () => {
            ...
            document.getElementById('mojang_status_icon').style.color = color
...
function asyncSystemScan(mcVersion, launchAfter = true) {
    ...
    sysAEx.on('message', (m) => {
        ...
        if (m.context === 'validateJava') {
            if (m.result == null) {
                ...
                setOverlayContent(
                    'Nenhuma instalação compatível<br> com Java encontrada',
                    'Para ingressar na Eteryun Network, você precisa de uma instalação de 64 bits do Java 8. Deseja que instalemos uma cópia? Ao instalar, você aceita o <a href="http://www.oracle.com/technetwork/java/javase/terms/license/index.html">contrato de licença da Oracle </a>.',
                    'Instalar Java',
                    'Instalar manualmente'
                )
                setOverlayHandler(() => {
                    setLaunchDetails('Preparando o download do Java..')
                ...
                setDismissHandler(() => {
                    $('#overlayContent').fadeOut(250, () => {
                        ...
                        setOverlayContent(
                            'Java é necessário <br> para iniciar',
                            'É necessária uma instalação x64 válida do Java 8 para iniciar.',
                            'Compreendo',
                            'Voltar'
                        )
                        setOverlayHandler(() => {
            ...
            if (m.result === true) {
                ...
                setLaunchDetails('Baixando Java..')
                ...
                setOverlayContent(
                    'Problema inesperado:<br> falha no download do Java',
                    'Infelizmente, encontramos um problema ao tentar instalar o Java. Você precisará instalar manualmente uma cópia.',
                    'Compreendo'
                )
                setOverlayHandler(() => {
                    toggleOverlay(false)
    ...
    setLaunchDetails('Verificando informações do sistema..')
...
const SETTING_USER = /^.+: Setting user:.+/
...
function dlAsync(login = true) {
    ...
    aEx.on('error', (err) => {
        ...
        showLaunchFailure('Erro durante a inicialização', err.message || 'Veja console (CTRL + Shift + i) para mais detalhes.')
    })
    aEx.on('close', (code, signal) => {
        if (code !== 0) {
            loggerLaunchSuite.error(`AssetExec exited with code ${code}, assuming error.`)
            showLaunchFailure('Erro durante a inicialização', 'Veja console (CTRL + Shift + i) para mais detalhes.')
        }
    })
        ...
        if (m.context === 'validate') {
            switch (m.data) {
                ...
                case 'version':
                    ...
                    setLaunchDetails('Validando integridade de ativos..')
                    break
                case 'assets':
                    setLaunchPercentage(60, 100)
                    loggerLaunchSuite.log('Asset Validation Complete')
                    setLaunchDetails('Validando a integridade da biblioteca..')
                    break
                case 'libraries':
                    setLaunchPercentage(80, 100)
                    loggerLaunchSuite.log('Library validation complete.')
                    setLaunchDetails('Validando integridade de arquivos diversos..')
                    break
                case 'files':
            ...
            switch (m.data) {
                case 'download':
                    ...
                    if (m.error.code === 'ENOENT') {
                        showLaunchFailure(
                            'Erro de download',
                            'Não foi possível conectar ao servidor de arquivos. Verifique se você está conectado à Internet e tente novamente.'
                        )
                    } else {
                        showLaunchFailure(
                            'Erro de download',
                            'Verifique o console (CTRL + Shift + i) para obter mais detalhes. Por favor, tente novamente.'
                        )
                    }
            ...
            if (m.result.forgeData == null || m.result.versionData == null) {
                ...
                showLaunchFailure('Erro durante a inicialização', 'Verifique o console (CTRL + Shift + i) para obter mais detalhes.')
            ...
            if (login && allGood) {
                ...
                loggerLaunchSuite.log(serv, versionData, forgeData, authUser, remote.app.getVersion())
                ...
                const tempListener = function (data) {
                    if (GAME_LAUNCH_REGEX.test(data.trim()) || SETTING_USER.test(data) || SETTING_USER.test(data.trim())) {
                ...
                const gameStateChange = function (data) {
                    ...
                    if (SERVER_JOINED_REGEX.test(data)) {
                        DiscordWrapper.updateDetails('Explorando o Reino!')
                    } else if (GAME_JOINED_REGEX.test(data)) {
                        DiscordWrapper.updateDetails('Navegando para Eteryun Network!')
                    }
                }
                ...
                const gameErrorListener = function (data) {
                    ...
                    if (data.indexOf('Could not find or load main class net.minecraft.launchwrapper.Launch') > -1) {
                        ...
                        showLaunchFailure('Erro durante a inicialização', 'O arquivo principal, LaunchWrapper, falhou ao baixar corretamente. Como resultado, o jogo não pode ser iniciado. <br> <br> Para corrigir esse problema, desligue temporariamente o software antivírus e inicie o jogo novamente.')
                    }
                }
                ...
                try {
                    ...
                    setLaunchDetails('Feito. Aproveite o servidor!')
    ...
    refreshDistributionIndex(true, (data) => {
        ...
        refreshDistributionIndex(false, (data) => {
            ...
            if (DistroManager.getDistribution() == null) {
                showLaunchFailure('Erro fatal', 'Não foi possível carregar uma cópia do índice de distribuição. Veja o console (CTRL + Shift + i) para mais detalhes.')
