...
function showMainUI(data) {
    ...
    setTimeout(() => {
        ...
        setInterval(() => {
            ...
            document.body.style.backgroundImage = `url('assets/images/backgrounds/${count}.png')`
            count++
            if (count > parseInt(document.body.getAttribute('backgrounds'))) {
                count = 1
            }
        }, 9000)
...
function showFatalStartupError() {
    setTimeout(() => {
        $('#loadingContainer').fadeOut(250, () => {
            ...
            setOverlayContent(
                'Erro fatal: Incapaz de carregar o índice de distribuição',
                'Não foi possível estabelecer uma conexão com nossos servidores para fazer download do índice de distribuição. Nenhuma cópia local estava disponível para carregar. <br> <br> O índice de distribuição é um arquivo essencial que fornece as informações mais recentes do servidor. O Launcher não pode ser iniciado sem ele. Verifique se você está conectado à Internet e reinicie o aplicativo.',
                'Fechar'
            )
            setOverlayHandler(() => {
    ...
    if (selectedAcc != null) {
        ...
        if (!val) {
            ...
            setOverlayContent(
                ...
                `Não foi possível atualizar o login de <strong>${selectedAcc.displayName}</strong>. Por favor ${accLen > 0 ? 'selecione outra conta ou ' : ''} faça o login novamente.`,
                'Logar',
